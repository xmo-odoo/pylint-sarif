...
to output e.g. both a machine-readable report to a file and a human-readable
output to stdout.
...
pylint has no requirement of (and no clean way to require) consistent
resolution roots: `path` is just relative to the cwd, and there is no
requirement to have project-level files to use pylint. This makes the use of
relative uris dodgy, but absolute uris are pretty much always broken for the
purpose of *interchange* so they're not really any better.
