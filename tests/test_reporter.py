...
from pylint_sarif import SCHEMA_URL, SCHEMA_VERSION, SARIFReporter
                        ...
                        "rules": [
                            {
                                ...
                                "deprecatedNames": [],
                                "shortDescription": {
                                    "text": "Used when a line is longer than a"
                                ...
                                "help": {
                                    "text": ":line-too-long (C0301):"
                                    " *Line too long (%s/%s)*\n  Used when a"
                                    " line is longer than a given number of"
                                    " characters."
                                },
...
def test_sarif_schema(pytestconfig: pytest.Config) -> None:
    jsonschema = pytest.importorskip(
        "jsonschema",
        reason="`jsonschema` is necessary to validate documents against the"
        " official SARIF schema",
    )
    schema = pytestconfig.cache.get(CACHE_KEY, None)
    if schema is None:
        try:
            res = urllib.request.urlopen(SCHEMA_URL, timeout=5)
        except urllib.error.URLError as e:
            raise pytest.skip(
                f"Unable to retrieve schema v{SCHEMA_VERSION}: {e.reason}"
            ) from e
    ...
    output.seek(0)
    jsonschema.validate(json.load(output), schema)
